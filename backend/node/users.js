...
async function generateAccessToken(data,secret,expirationTimeSeconds){
	...
	if(expirationDate == undefined) return jwt.sign(data,secret); //if no expiration date is specified, return token without expiration
	...
	jwt.verify(token,process.env.ACCESS_TOKEN_SECRET, (err,username)=>{
		...
		const obj = fs.readFile('../validkeys.json', (err,data)=>{
			...
			else
			{
				...
				if(!(obj.hasOwnProperty(request.body.user.username))) return res.sendStatus(401); //if not found then send status code UNAUTHORIZED.
			}
		});
...
async function checkForumPassword(username,pwdhash,callback){
	...
	client.query("SELECT pwd_hash FROM FORUMS WHERE forum_name= $1 ;",[username],(err,res)=>{
		...
		{
			if(res.rowCount === 1 && res.rows[0].pwd_hash===pwdhash) //compare password hashes
			{
				client.end();
