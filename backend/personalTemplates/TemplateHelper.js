...
async function checkTemplateExists(forum_name, templateName)
{
	...
	return new Promise((resolve, reject)=>{
		...
		client.connect();
		client.query('SELECT template_name from personal_templates WHERE forum_name=$1 AND template_name=$2;',[forum_name, templateName])
		.then(data=>{
			client.end();
...
async function insertNewTemplate(forum_name,templateName, filepath)
{
	...
	return new Promise((resolve, reject)=>{
		...
		.then(exists=>{
			if(exists) return resolve("A template already exists with the same name!");
			return checkPlaceHolders(filepath);
		})
		.then(okay=>{
			if(!okay) return resolve("Placeholders are invalid in the template!");
			return client.query('INSERT INTO personal_templates(forum_name, template_name, filepath) VALUES ($1,$2,$3)',[forum_name, templateName, filepath]);
		})
		.then(state=>{
			client.end();
...
async function fetchForumTemplates(forum_name)
{
	...
	return new Promise((resolve, reject)=>{
		var client = new Client();
		client.connect();
		client.query('SELECT template_name from personal_template WHERE forum_name=$1',[forum_name])
		.then(data=>{
			var arr = [];
			data.rows.forEach(row=>{arr.push(row.template_name)});
			client.end();
			return resolve(arr);
		})
		.catch(error=>{
