...
app.post("/checkRegistrationStatus", (req, res) => {
  try
  {
	...
	if(!queryUsername){return res.status(400).json({message:'Username unspecified in query'});}
	else{
  ...
  catch(err){return res.status(400).json({message:'BAed REeKset'});}
});
...
app.post("/registerForum", (req, res) => {
	try
	{
		...
		if(!data) return res.status(400).json({message:'No registration data found!'});
		else
